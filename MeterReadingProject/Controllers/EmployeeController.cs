...
namespace MeterReadingProject.Controllers
{
    public class EmployeeController : Controller
	{
        ...
        [HttpGet]
        public ActionResult Show1(int ID)
        {
            //var BranchName1 = HttpContext.Session.GetString("branch_name");

            var meterreadinglist = employeeRL.ShowReport1(ID);
            ViewBag.Model = meterreadinglist;
            return View(meterreadinglist);
            //var BranchName1 = HttpContext.Session.GetString("branch_name");
            //var meterlist = employeeRL.ShowReport(BranchName1,ID);
        ...
        public FileResult Export(int ID)
        {
            ...
            dt.Columns.AddRange(new DataColumn[6] {
                new DataColumn("Date"),
                new DataColumn("Meter Name"),
                new DataColumn("Meter Readings"),
                new DataColumn("Branch"),
                new DataColumn("Units"),
                new DataColumn("Cost")
            });

            var data = from emp in employeeRL.ShowReport(BranchName1, ID).ToList() select emp;

            foreach(var emp in data)
            {
                dt.Rows.Add(emp.date,emp.MeterName1,emp.MeterReading,emp.BranchName1,emp.units,emp.units*5);
            }
        ...
        public FileResult Export1(int ID)
        {
            ...
            var data = from emp in employeeRL.ShowReport1(ID).ToList() select emp;
